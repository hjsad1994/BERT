...
def load_config(config_path='dual-task-learning/config_multi.yaml'):
    """Load configuration"""
...
def generate_predictions(model, dataloader, device, aspect_names, sentiment_names, test_df, aspect_threshold=0.5):
    """
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Generating predictions"):
            ...
            # Simultaneous prediction: Aspect Detection + Sentiment Classification
            aspect_logits, sentiment_logits = model(input_ids, attention_mask)
            aspect_probs = torch.sigmoid(aspect_logits)
            aspect_present = (aspect_probs > aspect_threshold).long()
            sentiment_preds = torch.argmax(sentiment_logits, dim=-1)
            
            # Convert to CPU numpy
            present_np = aspect_present.cpu().numpy()
            preds_np = sentiment_preds.cpu().numpy()
            ...
            for i in range(batch_size):
                ...
                for j, aspect in enumerate(aspect_names):
                    ...
                    # Output sentiment prediction regardless of AD (simultaneous)
...
def main(args):
    ...
    if args.model_dir:
        ...
        possible_dirs = [
            default_dir,
            'dual-task-learning/models/dual_task_learning',
            'dual-task-learning/models/dual_task_learning-o'
        ]
        ...
        if model_dir is None:
            ...
            for dir_path in possible_dirs:
                print(f"   - {dir_path}")
            print("\nPlease train a dual-task model first, e.g.:")
            print("   python dual-task-learning/train_multitask.py --config dual-task-learning/config_multi.yaml")
            sys.exit(1)
    
    test_file = config['paths']['test_file']
    output_file = args.output if args.output else 'dual-task-learning/results/test_predictions_multi.csv'
    ...
    threshold = config.get('inference', {}).get('aspect_threshold', 0.5)
    pred_df, true_df = generate_predictions(
        model,
        test_loader,
        device,
        aspect_names,
